...
half power. For other pad types available see ImageFiltering documentation. As of doc writing, pad options
are: "replicate" (repeat edge values to infinity), "circular" (image edges "wrap around"), "symmetric" 
(the image reflects relative to a position between pixels), "reflect" (the image reflects relative 
to the edge itself).
"""
