...
"""
    boltzintegral(model::Function, time_series::Array{Float64,1}, params::Array{Float64,1}, prescribed_dot::Array{Float64,1}[; singularity = false])
...
- `model`: The viscoelastic relaxtion/creep modulus to be used
...
- `singularity`: set to 'true' if viscoelastic modulus has a singularity at t = 0
"""
function boltzintegral(model::Function, time_series::Array{Float64,1}, params::Array{Float64,1},
    ...
    if singularity
        ...
        for (i,v) in enumerate(time_series[2:end])
            ...
            Modulus_arg = v - τ
            Modulusᵢ = model(Modulus_arg, params)
...
"""
    boltzconvolve(model::Function, time_series::Array{Float64,1}, dt_series::Array{Float64,1}, params::Array{Float64,1}, prescribed_dot::Array{Float64,1}; singularity=false)
...
- `singularity`: set to 'true' if viscoelastic modulus has a singularity at t = 0
"""
function boltzconvolve(model::Function, time_series::Array{Float64,1}, dt_series::Array{Float64,1},
                        params::Array{Float64,1}, prescribed_dot::Array{Float64,1};
                        singularity = false)::Array{Float64,1}
    ...
    if singularity
        ...
        Modulus = model(time_series, params)
...
- `model`: Viscoelastic modulus function
...
- `_singularity`: Declare whether there is a singularity in viscoelastic modulus at t=0, true or false
...
function objectivefunc(params::Array{Float64,1}, grad::Array{Float64,1},
                            ...
                            measured; _singularity = false, _insight = false,
                            _sampling = "constant")

    # convolution if constant, brute force integration if variable
    if _sampling == "constant"
        convolved = boltzconvolve(model, time_series, dt_series, params, prescribed_dot; singularity = _singularity)
    elseif _sampling == "variable"
        convolved = boltzintegral(model, time_series, params, prescribed_dot; singularity = _singularity)
    end
...
"""
    leastsquares_init(params_init, low_bounds, hi_bounds, model, time_series, dt_series, prescribed_dot, measured; singularity = false, sampling = "constant", insight = false)
...
function leastsquares_init(params_init, low_bounds, hi_bounds,
                            model, time_series, dt_series, prescribed_dot,
                            measured; singularity = false, sampling = "constant",
                            insight = false)
