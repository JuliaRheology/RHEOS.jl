...
Compute the cost function to minimize during parameter fitting, Differential approach.
"""
function obj_const(params, equation, time_series, dt, strain, stress,data_struct, prob, ws, fittype::Differential;_insight::Bool = false)
...
"""
    Handle constraints given as an expression (assumed < 0) to
    adapt it to nlopt form required
"""
...
function leastsquares_init(params_init::Vector{RheoFloat},
    ...
    min_objective!(opt, (params, grad) -> obj_const(params, equation,
                                time_series, dt, 
                                strain,  
                                stress, data_struct, prob, ws,fittype;
                                _insight = insight))
