...
# A more optimal solution is to send constant sample rate data to the fitting function, but ask RHEOS to optimize with a focus on specific set of points. In this way, RHEOS will be able to benefit from the much faster and more accurate constant sample rate model simulation methods.
...
# For practical situations, when working with data embedded in `RheoTimeData` structs, RHEOS has a convenience function [`indexweight`](@ref) which makes it easy to generate a weightings array that can then be passed to either [`modelfit`](@ref) or [`modelstepfit`](@ref). The full description of [`indexweight`](@ref) can be found in the [API](@ref) section. Below, we include a typical example of its use, based on the same data used in [Fitting and Predicting - Time Data](@ref). 
